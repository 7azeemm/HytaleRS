use crate::config::ConnectionTimeouts;
use crate::net::auth::server_auth_manager::ServerAuthManager;
use crate::net::connection_manager::{Connection, ConnectionContext};
use crate::net::handlers::initial_handler::InitialPacketHandler;
use crate::net::utils::client_verifier::ClientVerifier;
...
pub const PROTOCOL_CRC: i32 = 1080406952;
...
impl ServerNetworkManager {
    pub async fn init() {
        ...
        let (endpoint, cert) = Self::create_endpoint()
            .await
    ...
    pub async fn create_endpoint() -> Result<(Endpoint, CertificateDer<'static>), Box<dyn Error>> {
...
async fn handle_connection(connecting: Connecting) {
    ...
    let client_cert = match connection
        .peer_identity()
        .map(|p| p.downcast::<Vec<CertificateDer>>())
    {
        Some(Ok(cert)) => *cert,
        _ => {
            error!(
                "Connection rejected: no client certificate from {}",
                remote_addr
            );
            return;
        }
    ...
    match timeout(ConnectionTimeouts::stream_timeout(), connection.accept_bi()).await {
        Ok(Ok((send, recv))) => {
            ...
            let rate_limiter = {
                let config = HytaleServer::get().config.read().await;
                RateLimiter::new(config.rate_limit.max_tokens, config.rate_limit.refill_rate)
            };
        ...
        Ok(Err(ConnectionError::ConnectionClosed(_))) => {
