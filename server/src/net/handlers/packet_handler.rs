...
macro_rules! handle_packet {
    ($self:expr, $packet_type:ty, $data:expr, $method:ident, $cx:expr) => {{
        match $crate::net::utils::packet_io::decode::<$packet_type>($data) {
            Some(packet) => $self.$method(packet, $cx).await,
            None => $crate::net::handlers::packet_handler::HandlerAction::Disconnect(format!(
                "Failed to decode packet {}",
                <$packet_type as protocol::io::packet::Packet>::name()
            )),
        }
    }};
}
