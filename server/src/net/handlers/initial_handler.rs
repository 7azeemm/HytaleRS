...
impl PacketHandler for InitialPacketHandler {
    ...
    async fn register(&mut self, cx: &mut ConnectionContext) {
        cx.set_timeout(HytaleServer::get().config.read().await.timeouts.initial)
            .await;
    }
}

impl InitialPacketHandler {
    async fn handle_connect(&self, packet: Connect, _cx: &mut ConnectionContext) -> HandlerAction {
        ...
        if matches!(packet.client_type, ClientType::Game) {
            ...
            return HandlerAction::Transition(Box::new(HandshakePacketHandler { connect: packet }));
        }
