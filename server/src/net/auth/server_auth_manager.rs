use crate::net::auth::credential_store::CredentialStore;
use crate::net::auth::jwt_validator::JWTValidator;
use crate::net::auth::services::auth_service::AuthService;
use crate::net::auth::services::session_service::{GameProfile, GameSession, SessionService};
use ahash::HashMap;
use base64::Engine;
...
impl ServerAuthManager {
    ...
    async fn auth(&self) {
        ...
        if !self.credential_store.has_tokens().await {
            self.set_auth_state(AuthState::Failed(AuthError::NoCredentials))
                .await;
            info!(
                "No Server credentials found, please run /auth command to authenticate the server."
            );
            return;
        }
        ...
        let Some(access_token) = self.refresh_auth_tokens().await else {
            self.set_auth_state(AuthState::Failed(AuthError::RefreshFailed))
                .await;
            return;
        };
        ...
        let Some(profiles) = self
            .session_service
            .fetch_game_profiles(&access_token)
            .await
        else {
            self.set_auth_state(AuthState::Failed(AuthError::NetworkError))
                .await;
            return;
        };
        ...
        self.credential_store
            .set_profile(profile.uuid.clone())
            .await;

        let Some(game_session) = self
            .session_service
            .create_game_session(&access_token, &profile.uuid)
            .await
        else {
            self.set_auth_state(AuthState::Failed(AuthError::NetworkError))
                .await;
...
fn schedule_game_session_refresh(session: &GameSession, session_token: String) {
    ...
    tokio::spawn(async move {
        ...
        match manager
            .session_service
            .refresh_game_session(&session_token)
            .await
        {
            None => {
                manager
                    .set_auth_state(AuthState::Failed(AuthError::RefreshFailed))
                    .await
            }
