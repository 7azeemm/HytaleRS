...
use protocol::packets::connection::{AuthGrant, AuthToken, ClientType, Connect, HostAddress, ServerAuthToken};
use crate::auth::jwt_validator::compute_certificate_fingerprint;
use crate::auth::server_auth_manager::ServerAuthManager;
use crate::connection_manager::ConnectionContext;
use crate::handle_packet;
use crate::handlers::setup_handler::SetupHandler;
use crate::packet::packet_handler::{HandlerAction, PacketHandler};
use crate::server_network_manager::SERVER_NETWORK_MANAGER;
...
impl PacketHandler for HandshakePacketHandler {
    ...
    async fn register(&mut self, cx: &mut ConnectionContext) {
        cx.set_timeout(HytaleServer::get().config.read().await.timeouts.auth).await;
...
impl HandshakePacketHandler {
    async fn request_auth_grant(&self, identity_token: String, cx: &mut ConnectionContext) {
        cx.set_timeout(HytaleServer::get().config.read().await.timeouts.auth_grant).await;
        ...
        cx.set_timeout(HytaleServer::get().config.read().await.timeouts.auth_token).await;
    }
    ...
    async fn handle_auth_token(&self, packet: AuthToken, cx: &mut ConnectionContext) -> HandlerAction {
        ...
        cx.set_timeout(HytaleServer::get().config.read().await.timeouts.auth_server_exchange).await;
